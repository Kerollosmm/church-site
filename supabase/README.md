...
Files 8–9 extend the schema beyond `BACKEND_AND_DATA_SPEC.md`: they carry the data layer of the
events/taxonomy/i18n feature and are mirrored column-for-column in `src/types/database.types.ts`
(5 new ENUM entries + 7 new table entries, including `event_series.default_term_ids`, an array whose
elements are validated by the application because PostgreSQL cannot express an FK over array
elements). The repository that reads them lives in `src/lib/store/supabase-driver.ts`.
