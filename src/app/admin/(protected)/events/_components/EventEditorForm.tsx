...
import {
  ...
  type EventFormState,
} from "@/lib/events/admin-form";
import type { AdminCapabilities } from "@/lib/domain/capabilities";
import { EVENT_STATUSES, EVENT_STATUS_LABELS_AR, type EventStatus } from "@/lib/domain/types";
      ...
      {seriesId ? (
        <div className="flex items-start gap-2 rounded-2xl border border-copticNavy-200 bg-copticNavy-50 p-4 text-xs text-copticNavy-900">
          ...
          <p className="leading-relaxed">
            ...
            <Link href={`/admin/events/series/${seriesId}`} className="font-bold underline">
              شاشة السلسلة
            </Link>
            .
          </p>
