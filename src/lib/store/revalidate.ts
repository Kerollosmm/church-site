...
// Cache invalidation for every events surface — the "no rebuild required" half of the design.
//
...
//
// Tag names come from `src/lib/tags.ts` — the repository's tag registry — never from a literal
// written here (see systemPatterns §1).

import { revalidatePath, revalidateTag } from "next/cache";
import { REVALIDATION_TAGS } from "@/lib/tags";
import { getEventRepository, type RepositoryDriverName } from "@/lib/store";
