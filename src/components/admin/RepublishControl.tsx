...
// src/components/admin/RepublishControl.tsx
// "آخر تحديث" + the manual cache clear, in one control.
//
// WHAT IT ACTUALLY DOES: calls `republishEventsAction()`, which drops every events cache entry and
// answers with the store's last-mutation instant. It is NOT a "save" — nothing about the content
// changes — so the button says so ("مسح الذاكرة المؤقتة وإعادة النشر") and the returned instant is
// labelled "آخر تعديل على المحتوى" rather than "آخر تحديث للموقع".
//
// A VIEWER SEES THE TIMESTAMP AND NO BUTTON: the control is capability-driven, and the action would
// refuse anyway (the refusal is recorded in the audit log — see the module header of
  ...
  const run = () => {
    startTransition(async () => {
      ...
      if (answer.success) {
        ...
        // The public pages are re-fetched on their next request; refreshing here updates the admin's
        // own server-rendered numbers (occurrence previews, counts) in place.
        router.refresh();
      } else {
