...
│   ├── /clinics/doctors (دليل الأطباء والمواعيد التفصيلية)
│   └── /clinics/status (نبض العيادات اللحظي والاعتذارات الطارئة)
│
...
5. **شريط التنبيهات ونبض الخدمة (Dynamic Status / Live Alert Strip)**:
   - مساحة مخصصة لأي مستجدات طارئة (اعتذار طبيب، تغيير موعد قداس، أو موعد بدء التسجيل).
...
| :- | :--- | :--- | :--- | :--- |
| **01** | `/` | الرئيسية - نبض الكنيسة | **ISR (Revalidate: 60s)** | `HeroBanner`, `NextMassCountdown`, `DoctorAlertTicker`, `QuickServiceGrid`, `LatestNewsCarousel`, `BibleVerseDaily` |
...
| **06** | `/clinics` | المستوصف الطبي التخصصي | **ISR (Revalidate: 60s)** | `ClinicHero`, `SpecialtyBrowser`, `DoctorSearchInput`, `RealtimeAbsenceStrip`, `BookingInstructions` |
| **07** | `/clinics/specialties` | دليل التخصصات الطبية | **SSG (Static)** | `SpecialtyCard`, `DoctorsBySpecialtyList`, `RoomBadge`, `EquipmentShowcase` |
| **08** | `/clinics/doctors` | جدول مواعيد الأطباء | **ISR (Revalidate: 60s)** | `DoctorScheduleMatrix`, `DoctorProfileModal`, `AbsenceIndicator`, `CallClinicAction` |
| **09** | `/clinics/status` | نبض العيادات والاعتذارات | **SSR / Edge Dynamic** | `LiveAbsenceFeed`, `DoctorAbsenceList`, `ClinicNotificationBell`, `EmergencyHotlineCard` |
| **10** | `/meetings/malaeika` | اجتماع الملائكة (حضانة) | **SSG (Static)** | `MeetingHero`, `AgeCurriculumTimeline`, `HymnAudioSnippet`, `ParentWhatsAppLink` |
| **11** | `/meetings/ebtedaey-1-3` | اجتماع ابتدائي صغار | **SSG (Static)** | `ActivityCardGrid`, `WeeklyVerseBox`, `ServantsTeamBadge`, `MeetingScheduleCard` |
| **12** | `/meetings/ebtedaey-4-6` | اجتماع ابتدائي كبار | **SSG (Static)** | `BibleCompetitionCard`, `WeeklyLessonPlan`, `MeetingScheduleCard`, `AttendanceRewards` |
| **13** | `/meetings/edaady` | اجتماع المرحلة الإعدادية | **SSG (Static)** | `YouthHeroBanner`, `TopicDiscussionCard`, `YouthServantsList`, `TelegramChannelBtn` |
| **14** | `/meetings/thanaway` | اجتماع المرحلة الثانوية | **SSG (Static)** | `SpiritualGuidanceBox`, `WeeklyThemeCard`, `YouthRetreatGallery`, `ScheduleBadge` |
| **15** | `/meetings/arshi-youth` | أسرة شباب عرشي الجامعي | **ISR (Revalidate: 1800s)** | `UniversityYouthHero`, `SpiritualLecturesArchive`, `TripRegistrationCard`, `CommunityAction` |
| **16** | `/meetings/ni-angelos` | اجتماع ني أنجيلوس (خريجين) | **ISR (Revalidate: 1800s)** | `GraduatesHero`, `ProfessionalFellowship`, `CareerMentorship`, `MeetingTimeCard` |
| **17** | `/meetings/holy-family` | اجتماع الأسرة المقدسة | **SSG (Static)** | `FamilyCounselingCard`, `ParentingWorkshops`, `CouplesRetreatAnnouncement` |
| **18** | `/meetings/men` | اجتماع رجال القديس يوسف | **SSG (Static)** | `MenMeetingHero`, `SpiritualTalksSchedule`, `SocialCareProgram`, `ScheduleCard` |
| **19** | `/meetings/om-elkhalas` | اجتماع أم الخلاص للسيدات | **SSG (Static)** | `MothersHero`, `FamilyPrayerCard`, `WeeklyHomilyAudio`, `MeetingSchedule` |
| **20** | `/meetings/widows-orphans` | اجتماع مارمينا والبابا كيرلس | **SSG (Static)** | `CompassionServiceHero`, `AssistanceProgramDetails`, `ComfortTalksArchive` |
| **21** | `/education/deacon-school` | مدرسة القديس إستفانوس | **ISR (Revalidate: 300s)** | `DeaconLevelTabs`, `RegistrationCountdownTimer`, `HymnAudioPlayer`, `DeaconEnrollmentForm` |
| **22** | `/education/children-bible`| مدرسة الكتاب المقدس للأطفال | **SSG (Static)** | `InteractiveStoryTimeline`, `BibleQuizCard`, `ColoringDownloadBox` |
| **23** | `/education/adult-bible` | معهد الكتاب المقدس للكبار | **SSG (Static)** | `FourYearAcademicCurriculum`, `LectureScheduleTable`, `EnrollmentRequirements`, `StudyMaterials` |
| **24** | `/education/karouz-academy`| معهد كاروز لإعداد الخدام | **SSG (Static)** | `AcademySectionsGrid`, `AdmissionGuidelines`, `ServantPledgeCard`, `ApplicationDownload` |
| **25** | `/education/cithara-choir` | قيثارة التسابيح والكورال | **SSG (Static)** | `ChoirAudioShowcase`, `AuditionDatesCard`, `HymnTrackList`, `PerformancesGallery` |
| **26** | `/activities/scouts` | فوج الكشافة والمرشدات | **SSG (Static)** | `ScoutRanksHierarchy`, `CampPhotoGallery`, `ScoutRegistrationTerms`, `PatrolLeaderProfiles` |
| **27** | `/activities/lending-library`| مكتبة الاستعارة العامة | **ISR (Revalidate: 3600s)** | `BookCategoryFilter`, `CatalogSearchInput`, `BorrowingRulesModal`, `BookRecommendationCard` |
| **28** | `/activities/printing-center`| مركز الطباعة والتصوير | **SSG (Static)** | `ServicePriceTable`, `DocumentUploadPrompt`, `WorkingHoursBadge`, `DirectWhatsAppOrder` |
| **29** | `/activities/creativity-center`| مركز تنمية المواهب | **SSG (Static)** | `WorkshopCatalog`, `TalentExhibitionGallery`, `RegistrationFormModal` |
| **30** | `/activities/social-club` | النادي الاجتماعي والرياضي | **SSG (Static)** | `SportsFacilitySchedule`, `TournamentFixtures`, `ClubRulesCard`, `CoachProfiles` |
| **31** | `/activities/computer-center`| مركز التكنولوجيا والكمبيوتر | **SSG (Static)** | `CourseCatalogGrid`, `UpcomingWorkshops`, `CertificateAccreditation`, `EnrollAction` |
| **32** | `/activities/summer-club` | النادي الصيفي السنوي | **ISR (Revalidate: 1800s)** | `SummerProgramHero`, `DailyActivitiesAgenda`, `SummerPricingCard`, `OnlineRegistrationBtn` |
| **33** | `/services/educational-center`| المركز التعليمي الدراسي | **SSG (Static)** | `TutoringScheduleByGrade`, `TeacherDirectory`, `CenterAdmissionPolicy` |
| **34** | `/services/nursery` | حضانة القديسين النموذجية | **SSG (Static)** | `NurseryInfantProgram`, `PreSchoolCurriculum`, `DailyMealSchedule`, `NurseryTourModal` |
| **35** | `/services/canteens` | المقاصف والكانتين | **SSG (Static)** | `MenuDisplayCard`, `HygieneCertificationBadge`, `ServiceHoursCard` |
| **36** | `/services/charitable-kitchen`| المطبخ الخيري ومنفذ المحبة | **SSG (Static)** | `CharityMissionHero`, `FoodDriveSchedule`, `VolunteerSignUpCard`, `PackagingGuidelines` |
| **37** | `/services/employment-office`| مكتب التوظيف والتأهيل | **ISR (Revalidate: 600s)** | `JobVacanciesList`, `ResumeSubmissionForm`, `EmployerPartnersLogoGrid` |
| **38** | `/services/membership-registry`| مكتب السجل والشؤون الكنسية| **SSG (Static)** | `CertificateIssuanceGuide`, `RequiredDocumentsChecklist`, `OfficeHoursCard` |
| **39** | `/services/church-giftshop` | مكتبة البيع والهدايا | **SSG (Static)** | `ProductCategoryShowcase`, `FeaturedIconsGrid`, `GiftshopHoursBadge` |
| **40** | `/services/tailoring-workshop`| مشغل وتفصيل الملابس الكنسية| **SSG (Static)** | `VestmentCatalogGrid`, `TailoringOrderForm`, `FabricShowcaseCard` |
| **41** | `/condolence` | حجز قاعة العزاء | **SSR / Edge Form** | `CondolenceBookingForm`, `AvailabilityCalendar`, `RulesAccordion`, `ReservationTrackingCard` |
| **42** | `/live` | البث المباشر والصلوات | **SSR / Edge Live** | `LiveStreamPlayer`, `ActiveStreamStatusBadge`, `UpcomingStreamsTimeline`, `YouTubeSubscription` |
| **43** | `/bible` | قارئ الكتاب المقدس | **Client-Side SPA** | `TestamentSelector`, `BookNavigator`, `ChapterViewer`, `FulltextBibleSearch`, `FontResizer` |
| **44** | `/donations` | التبرعات والحسابات البنكية | **SSG (Static)** | `OfficialBankCard`, `IbanCopyWidget`, `DonationPurposeBreakdown`, `TransparencyNotice` |
| **45** | `/contact` | الاتصال والعنوان | **SSG (Static)** | `ContactForm`, `EmergencyClergyPhoneGrid`, `GoogleMapsInteractive`, `SocialLinksRow` |
...
erDiagram
    ...
    DOCTOR ||--o{ DOCTOR_ABSENCE : records
...
4. **العيادات الطبية**: تفرعات (دليل التخصصات، أطباء العيادات، شريط الاعتذارات اللحظي، مواعيد المعمل والصيدلية).
...
- **شريط الوصول السريع السفلي المثبت (Bottom Floating Action Bar)**:
  ...
  - أيقونة العيادات والاعتذارات (مع شارة تنبيهية حمراء في حال وجود اعتذار طبيب اليوم).
