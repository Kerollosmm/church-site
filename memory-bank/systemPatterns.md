...
## 5. هيكل المستودع
`src/app` (43 مساراً)، `src/actions` (Server Actions)، `src/lib/{supabase,queries,tags,validations,security}`، `docs/{adr,agents}`، `memory-bank/` (يُزامن بداية ونهاية كل مهمة).
