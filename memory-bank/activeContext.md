...
- **الحالة الراهنة**: `tsc --noEmit` = 0 أخطاء، و`pnpm run build` = 52/52 صفحة ثابتة + 4 مسارات إدارية ديناميكية، بلا أي متغيرات بيئة (البناء يعمل بالبذرة)، و`pnpm run lint` = 0 أخطاء و0 تحذيرات، و`pnpm audit --prod` = صفر ثغرات. هجرات القاعدة صارت **9 ملفات** (7 للأساس + 2 لنظام الفعاليات). وطبقة الفعاليات تعمل فعلياً بلا قاعدة عبر `.data/church-store.json` (مُهمَل في git). لم يُعمل أي commit — كل العمل في شجرة العمل.
...
1. **الخطوة 2 من ميزة الفعاليات: شرائح الواجهة فوق هذه الطبقة** — تُبنى على السطح الموجود بلا تعديله: صفحات عامة (`/events` وقائمة/تفاصيل) تقرأ عبر المستودع وتوسّع السلاسل بـ `expandSeries` وتُنسّق بـ `localized()`، وشاشة إدارية للفعاليات ومحررها (بما فيها إلغاء/نقل موعد واحد)، وشاشة سجل التدقيق، ثم تركيب `LocaleSwitcher` في `Header`. المسارات الجديدة تُضاف إلى `EVENT_SURFACE_PATHS` في `src/lib/store/revalidate.ts` وإلا لن يُبطل كاشها.
...
5. [متبقٍ من 4.3] **بند §9.3 رقم 7 غير مستوفى**: لا `src/app/sitemap.ts` ولا `src/app/robots.ts`، و`NEXT_PUBLIC_SITE_URL` لا يستهلكه أي كود (`git grep` = 0) — مطلوب قبل Claim النطاق وSearch Console.
6. [متبقٍ من P2.3] بناء الشاشات الإدارية غير المنفَّذة: صندوق رسائل التواصل، طلبات التسجيل والتوظيف، شريط التنبيهات، جدولة البث، ومحرر جداول القداسات/الاستثناءات الموسمية — وهي مُعلَّمة الآن صراحةً في لوحة التحكم. وبقيت معها سطحيات ميتة مقصودة: `refreshStreamData` و`submitJobApplication` و`getSiteAlerts`.
...
8. [متبقٍ] لم تُختبر الواجهة تفاعلياً في متصفح بمفتاح Turnstile حقيقي، ولم يُختبر تدفق اعتماد/اعتذار الحجز على قاعدة بيانات حقيقية (يتطلب البند 3 أولاً) — التحقق هنا بنائي (tsc/build/lint/HTML) وبقراءة كود فقط.
...
10. [متبقٍ من 3.5] لم يُشغَّل سير عمل CI على GitHub بعد (لا push ولا PR في هذه التمريرة)؛ صحة YAML والخطوات مُتحقَّقة محلياً فقط.
11. [متبقٍ من 3.3] الإصلاح الفعلي لثغرات `postcss` جاء من `pnpm.overrides` لأن `next@15.5.25` يثبّت `postcss@8.4.31`؛ متى رُفع `next` إلى minor أحدث يعتمد postcss مُصلَحاً يمكن حذف الـ override (لا يجب حذفه قبله، وإلا عادت الثغرات الأربع).
12. [طريقة عمل، من 4.1] عند إثبات حذف أي كود: استخدم `git grep` (أو `command grep`) لا `grep` المجرَّد — فالأخير دالة غلاف في هذه الصدَفة تُفشل `-E` مع `-n`/`-i`/`-c` وتُرجع «0 مطابقة» كاذبة. **وانتباه**: `git grep` لا يرى الملفات غير المتتبَّعة (الجديدة قبل `git add`)، ففحص المستهلكين في ملفات أُضيفت للتو يجب أن يكون بـ `command grep -rl` على شجرة العمل.
13. [متبقٍ من الخطوة 1] تحميل ملفات الوسائط فعلياً (bytes) لم يُنفَّذ: `media` يسجّل البيانات الوصفية فقط (`registerMediaAction`) ولا يوجد مخزن ملفات؛ و`src/lib/store/seed.ts` تُبقي `media` فارغة عن قصد لأن أصول البذرة (`/assets/...`) غير موجودة في المستودع.
14. [متبقٍ من الخطوة 1] تدقيق Supabase **بذل وسع لا معاملة** (نداءان منفصلان عبر PostgREST: الصف ثم سطر التدقيق)، بخلاف المحرك الملفي حيث الكتابة واحدة تغطي الاثنين. الحل النظيف: دالة `SECURITY DEFINER` واحدة لكل تعديل — مؤجَّلة عن قصد ومسجَّلة هنا.
