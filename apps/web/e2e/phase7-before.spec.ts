...
test.describe('Baseline Before Screenshots', () => {
  for (const vp of VIEWPORTS) {
    test(`Capture home at ${vp.label}`, async ({ page }) => {
      ...
      await page.screenshot({
        path: path.join(SCREENSHOTS_DIR, `before-home-${vp.label}.png`),
        fullPage: true,
      });
    });
    ...
    test(`Capture about at ${vp.label}`, async ({ page }) => {
      ...
      await page.screenshot({
        path: path.join(SCREENSHOTS_DIR, `before-about-${vp.label}.png`),
        fullPage: true,
      });
    });
    ...
    test(`Capture services at ${vp.label}`, async ({ page }) => {
      ...
      await page.screenshot({
        path: path.join(SCREENSHOTS_DIR, `before-services-${vp.label}.png`),
        fullPage: true,
      });
    });
    ...
    test(`Capture content at ${vp.label}`, async ({ page }) => {
      ...
      await page.screenshot({
        path: path.join(SCREENSHOTS_DIR, `before-content-${vp.label}.png`),
        fullPage: true,
      });
    });
    ...
    test(`Capture admin dashboard at ${vp.label}`, async ({ page }) => {
      ...
      await page.screenshot({
        path: path.join(SCREENSHOTS_DIR, `before-admin-dashboard-${vp.label}.png`),
        fullPage: true,
      });
    });
    ...
    test(`Capture admin videos at ${vp.label}`, async ({ page }) => {
      ...
      await page.screenshot({
        path: path.join(SCREENSHOTS_DIR, `before-admin-videos-${vp.label}.png`),
        fullPage: true,
      });
    });
  }
  ...
  test('Capture mobile nav drawer open state at 390', async ({ page }) => {
    ...
    await page.screenshot({
      path: path.join(SCREENSHOTS_DIR, 'before-nav-drawer-390.png'),
      fullPage: true,
    });
  });
});
